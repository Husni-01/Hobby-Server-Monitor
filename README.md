...
```bash
git clone https://github.com/<your-username>/Hobby-Server-Monitor.git
...
Create two systemd service files on your Ubuntu host:

**`/etc/systemd/system/lxd-monitor-api.service`**
```ini
[Unit]
Description=LXD Monitor API Server
After=network.target lxd.service

[Service]
User=ubuntu
WorkingDirectory=/home/ubuntu/Hobby-Server-Monitor/Backend
EnvironmentFile=/home/ubuntu/Hobby-Server-Monitor/Backend/.env
ExecStart=/home/ubuntu/Hobby-Server-Monitor/Backend/venv/bin/gunicorn app:app --bind 0.0.0.0:8000 --workers 2
Restart=always
RestartSec=5
...
**`/etc/systemd/system/lxd-monitor-collector.service`**
```ini
[Unit]
Description=LXD Monitor Metrics Collector
After=network.target lxd.service
...
WorkingDirectory=/home/ubuntu/Hobby-Server-Monitor/Backend
EnvironmentFile=/home/ubuntu/Hobby-Server-Monitor/Backend/.env
ExecStart=/home/ubuntu/Hobby-Server-Monitor/Backend/venv/bin/python collector.py
Restart=always
RestartSec=5
...
For production, build the Astro frontend (`npm run build`) and serve `dist/` with nginx or caddy, with a reverse proxy to the Falcon API on `localhost:8000`.
